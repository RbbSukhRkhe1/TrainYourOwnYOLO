...
if __name__ == "__main__":
    ...
    for item in input_paths:
        # Skip macOS metadata files (starting with ._) and __MACOSX directories
        # Check both the full path and basename for case-insensitive matching
        item_normalized = str(item).lower()
        item_basename = os.path.basename(item)
        
        # Skip if it's a macOS metadata file or in __MACOSX directory
    ...
    if input_image_paths and not webcam_active:
        ...
        for i, img_path in enumerate(input_image_paths):
            ...
            try:
                ...
                print(f"Skipping {img_path} - error during detection: {e}")
                continue
            
            # Skip if file couldn't be opened (detect_object returns None, None)
            if prediction is None or image is None:
                print(f"Skipping {img_path} - file could not be opened")
                continue
            
            # Get image dimensions - image is already a numpy array from detect_object
            try:
            ...
            # Process predictions (prediction can be empty list if no detections found)
            # This is valid - we just won't add any rows for this image
